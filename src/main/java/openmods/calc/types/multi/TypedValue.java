...
public class TypedValue {
	...
	private Optional<IMetaObject> metaObject;
	...
	TypedValue(TypeDomain domain, Class<?> type, Object value, IMetaObject metaObject) {
		Preconditions.checkArgument(type.isInstance(value), "Value '%s' is not instance of '%s'", value, type);
		this.domain = domain;
		this.type = type;
		this.value = value;
	...
	public IMetaObject getMetaObject() {
		if (!metaObject.isPresent()) {
			final IMetaObject defaultMetaObject = domain.getDefaultMetaObject(type);
